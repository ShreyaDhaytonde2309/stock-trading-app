        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', minHeight: '90vh', background: '#f4f7fa' }}>
            ...
            <div style={{ flex: 1, padding: isMobile ? '20px' : '30px 40px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    {filteredStocks.map(stock => (
                        <div key={stock._id} style={{ ...watchlistCardStyle, flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? '15px' : '30px' }}>
                            <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                {isMobile && (
                                    <button onClick={() => navigate(`/admin-stock/${stock.symbol}`)} style={{ ...viewChartBtn, padding: '8px 15px', fontSize: '0.8rem' }}>View Chart</button>
                                )}
                            </div>
                            <div style={{ flex: 2 }}>
                                <div style={labelSmall}>Stock name</div>
                    ...
                    {filteredStocks.length === 0 && <div style={{ textAlign: 'center', padding: '50px', color: '#94a3b8' }}>No stocks found.</div>}
                </div>
            </div>
        </div>
    );
...
const viewChartBtn = { background: '#07589e', color: 'white', padding: '10px 25px', borderRadius: '8px', border: 'none', fontWeight: '600', cursor: 'pointer' };
